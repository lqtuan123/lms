...
@section('content')
    ...
    <div class="intro-y flex items-center mt-8">
        <h2 class="text-lg font-medium mr-auto">
            Sửa người dùng đang học
        </h2>
    </div>
    <div class="grid grid-cols-12 gap-12 mt-5">
        <div class="intro-y col-span-12 lg:col-span-12">
            <!-- BEGIN: Form Layout -->
            <form method="post" action="{{route('admin.learning.update', $danghoc->id)}}" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                <div class="intro-y box p-5">
                    <div class="mt-3">
                        ...
                        <select id="user_id" name="user_id" class="form-select mt-2">
                            @php
                                $selectedId = $danghoc->user_id; // Lấy id của thời khóa biểu đang được điểm danh
                            @endphp
                    ...
                    <div class="mt-3">
                        ...
                        <select id = "phancong_id" name="phancong_id" class="form-select mt-2">
                            @php
                                $selectedId = $danghoc->phancong_id; // Lấy id của thời khóa biểu đang được điểm danh
                            @endphp
                    ...
                    <div class="mt-3">
                        ...
                        <select id="time-spending" name="time_spending" class="form-select mt-2" required>
                            @php
                                // Lấy giá trị thời gian học hiện tại
                                $selectedId = $danghoc->time_spending ?? null;
                            @endphp
                    
                            <option value="" disabled {{ is_null($selectedId) ? 'selected' : '' }}>Chọn thời gian học</option>
                            <option value="15" {{ $selectedId == 15 ? 'selected' : '' }}>15 phút</option>
                            <option value="30" {{ $selectedId == 30 ? 'selected' : '' }}>30 phút</option>
                            <option value="45" {{ $selectedId == 45 ? 'selected' : '' }}>45 phút</option>
                            <option value="60" {{ $selectedId == 60 ? 'selected' : '' }}>60 phút</option>
                            <option value="120" {{ $selectedId == 120 ? 'selected' : '' }}>120 phút</option>
                        </select>
                    </div>                    
                    
                    <div class="mt-3">
                        <label for="status" class="form-label">Trạng thái</label>
                        <select id="status" name="status" class="form-select mt-2" required>
                            @php
                                // Lấy trạng thái hiện tại, mặc định là 'started'
                                $selectedStatus = $danghoc->status ?? 'started';
                            @endphp
