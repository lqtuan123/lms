...
@section('content')
    ...
    <div class="intro-y flex items-center mt-8">
        <h2 class="text-lg font-medium mr-auto">
            Sửa chứng nhận hoàn thành khóa học
        </h2>
    </div>
    <div class="grid grid-cols-12 gap-12 mt-5">
        <div class="intro-y col-span-12 lg:col-span-12">
            <!-- BEGIN: Form Layout -->
            <form method="post" action="{{route('admin.enrollcertificates.update', $chungnhan->id)}}" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                <div class="intro-y box p-5">
                    <div class="mt-3">
                        ...
                        <select id="user_id" name="user_id" class="form-select mt-2">
                            @php
                                $selectedId = $chungnhan->user_id; // Lấy id của thời khóa biểu đang được điểm danh
                            @endphp
                            @foreach ($user as $data)
                                <option value="{{ $data->id }}" {{ $selectedId == $data->id ? 'selected' : '' }}>{{ $data->full_name }}</option>
                            @endforeach
                        </select>   
                    </div>
                    ...
                    <div class="mt-3">
                        ...
                        <select id = "ketqua " name="ketqua" class="form-select mt-2">
                            @php
                                $selectedId = $chungnhan->ketqua; // Lấy id của thời khóa biểu đang được điểm danh
                            @endphp
                            @if ($selectedId == 0)
                                <option value="0">Đạt</option>
                                <option value="1">Rớt</option>
                            @else
                                <option value="1">Rớt</option>
                                <option value="0">Đạt</option>
                            @endif
                    ...
                    <div class="mt-3">
                        <label for="regular-form-1" class="form-label">Người cấp</label>
                        <select id = "nguoicap_id" name="nguoicap_id" class="form-select mt-2">
                            @php
                                $selectedId = $chungnhan->nguoicap_id; // Lấy id của thời khóa biểu đang được điểm danh
                            @endphp
                            @foreach ($teacher as $data)
                                <option value="{{ $data->id }}" {{ $selectedId == $data->id ? 'selected' : '' }}>{{ $data->mgv }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="mt-3">
                        <label for="regular-form-1" class="form-label">Đơn vị</label>
                        <select id = "donvi_id" name="donvi_id" class="form-select mt-2">
                            @php
                                $selectedId = $chungnhan->donvi_id; // Lấy id của thời khóa biểu đang được điểm danh
                            @endphp
                            @foreach ($donvi as $data)
                                <option value="{{ $data->id }}" {{ $selectedId == $data->id ? 'selected' : '' }}>{{ $data->title }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="mt-3">
                        ...
                        <select id = "phancong_id" name="phancong_id" class="form-select mt-2">
                            @php
                                $selectedId = $chungnhan->phancong_id; // Lấy id của thời khóa biểu đang được điểm danh
                            @endphp
                            @foreach ($phancong as $data)
                                <option value="{{ $data->id }}" {{ $selectedId == $data->id ? 'selected' : '' }}>Mã giảng viên: {{ $data->giangvien->mgv }}, Môn học: {{ $data->hocphan->title }}</option>
                            @endforeach
                        </select>
                    </div>

                    <div class="mt-3">
                        <label for="regular-form-1" class="form-label">Loại tín chỉ</label>
                        <select id = "loai_id" name="loai_id" class="form-select mt-2">
                            @php
                                $selectedId = $chungnhan->loai_id; // Lấy id của thời khóa biểu đang được điểm danh
                            @endphp
                            @foreach ($loaichungchi as $data)
                                <option value="{{ $data->id }}" {{ $selectedId == $data->id ? 'selected' : '' }}>{{ $data->title }}</option>
                            @endforeach
                        </select>
                    </div>
