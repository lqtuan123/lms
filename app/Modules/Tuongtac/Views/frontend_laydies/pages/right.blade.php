...
{
    $groupmembers = \DB::select('select a.* , b.full_name ,b.photo from (select * from group_members where group_id = '.$page->item_id.' ) as a left join users b on a.user_id = b.id');
                        ...
                        <li>
                            <a href=" ">
                                <div class="popular-post-item">
                                    <img src="{{  $member->photo }}" alt="{{ $member->full_name }}" class="popular-post-thumbnail">
                                    <div class="popular-post-title">
                                        <a href="{{TPage::getPageUrl($member->user_id,'user')}}">
                                            {{Str::limit($member->full_name, 60) }} {{$member->role!='member'?'('.$member->role.')':''}}
                                        </a>
                                    </div>
                                </div>
                            </a>
