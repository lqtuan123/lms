    ...
    <form action="{{ route('admin.event_type.store') }}" method="POST">
        ...
        <div class="form-group">
            ...
            <select name="location_type" class="form-control" required>
                <option value="outdoor" {{ old('location_type') == 'outdoor' ? 'selected' : '' }}>Ngoài trời</option>
                <option value="indoor" {{ old('location_type') == 'indoor' ? 'selected' : '' }}>Trong nhà</option>
            </select>
            @error('location_type')
                <div class="text-danger">{{ $message }}</div>
            @enderror
        ...
        <div class="form-group">
            ...
            <textarea name="location_address" class="form-control">{{ old('location_address') }}</textarea>
